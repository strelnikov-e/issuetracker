...
export default function Issues() {
  ...
  const IssueList = () => {
    return (
      <>
    {Object.values(data.issues).map((issue) => (
      <tr key={issue.id}>
        <td valign="middle" className="text-secondary">{issue.key}</td>
        <td valign="middle">
          <TypeBadge type={issue.type} />
        </td>
        <td valign="middle" as={Link}>
          <Link to={{
                pathname: `/issues/${issue.id}`,             
                }}
                state= { {projectId: issue.project.id} } className="link-dark">
            {issue.name}
          </Link>
        </td>
        ...
        <td valign="middle">{issue.startDate}</td>
        <td>
          <ButtonGroup>
            <Button
              size=""
